...
public class PlayerControllerNewJump : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            Debug.LogError("No GROUND PROJECTILE selected on SECONDARY gun for " + gameObject);
        }
    ...
    void Update()
    {
        ...
        if (!Input.GetButton("Jump") && groundJumpInitiated && (maxJumpTimer - currentJumpTimer <= fullJumpLimit))
        {
            ...
            Debug.Log("Short Jump " + (maxJumpTimer - currentJumpTimer));
            rb.AddForce(groundJumpForce, ForceMode.Impulse);
            groundJumpInitiated = false;
        }
        else if (Input.GetButton("Jump") && groundJumpInitiated && (maxJumpTimer - currentJumpTimer > fullJumpLimit))
        {
            // Do full jump
    ...
    void FixedUpdate()
    {
        rb.velocity = new Vector3(Mathf.Clamp(rb.velocity.x, -maxMoveSpeed, maxMoveSpeed), Mathf.Clamp(rb.velocity.y, -maxMoveSpeed, maxMoveSpeed), 0);
    ...
    private void JumpAttack()
    {
        ...
        newJumpMelee.transform.parent = transform;
    ...
    private void MeleeAttack()
    {
        ...
        switch (Constants.WhatsBelowMe(gameObject))
        {
            case "Ground":
                ...
                GameObject newGroundMelee = Instantiate(meleeObject, groundMeleeGun.position, groundMeleeGun.rotation);
                newGroundMelee.transform.parent = transform;
        ...
        switch (Constants.WhatsBelowMe(gameObject))
        {
            case "Ground":
                if (groundProjectile.name == "Player Projectile Lobbed")
                {
                    ...
                    newGroundProjectile.transform.parent = projectileParent.transform;
            ...
            default:
                ...
                newAirProjectile.transform.parent = projectileParent.transform;
